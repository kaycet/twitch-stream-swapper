...
class PopupManager {
  ...
  updateActionBadge(enabled) {
    try {
      ...
      // Same shape as the background worker's badge (shared helper), so
      // opening the popup doesn't stomp the live count until the next poll.
      const liveStreams = this.streams.filter((s) => s.isLive);
      const { text, color, title } = badgeState({
        enabled,
        liveCount: liveStreams.length,
        target: enabled ? (liveStreams[0]?.username || null) : null,
      });
